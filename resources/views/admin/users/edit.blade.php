...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label>{{ trans('cruds.user.fields.is_profile_completed') }}</label>
                @foreach(App\Models\User::IS_PROFILE_COMPLETED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_profile_completed') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_profile_completed_{{ $key }}" name="is_profile_completed" value="{{ $key }}" {{ old('is_profile_completed', $user->is_profile_completed) === (string) $key ? 'checked' : '' }}>
                        <label class="form-check-label" for="is_profile_completed_{{ $key }}">{{ $label }}</label>
                    </div>
                @endforeach
                @if($errors->has('is_profile_completed'))
                    <div class="invalid-feedback">
                        {{ $errors->first('is_profile_completed') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.user.fields.is_profile_completed_helper') }}</span>
            </div>
            <div class="form-group">
