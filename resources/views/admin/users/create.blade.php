            ...
            <div class="form-group">
                ...
                @foreach(App\Models\User::IS_PROFILE_COMPLETED_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('is_profile_completed') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="is_profile_completed_{{ $key }}" name="is_profile_completed" value="{{ $key }}" {{ old('is_profile_completed', '0') === (string) $key ? 'checked' : '' }}>
